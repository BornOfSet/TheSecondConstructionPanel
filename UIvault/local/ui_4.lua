...
----actions----
UI = CreateWindow(GetFrame(0),'Slot',nil,false,false,false,false,'VAULT_ui_4',Position,Border)
...
UI._closeBtn.OnClick = function(control)
	...
	local data = EntityCategoryGetUnitList(categories.TECH2 * categories.AEON)  --这个行动访问所有已经被加载了的蓝图，并且返回typeclass名称如xsl0001等
	...
	for c,id in data do
		...
		UI.Images[c].cursedbtn = CreateButton(UI.Images[c],
		...
		UI.Images[c].cursedbtn.OnClick = function(self, modifiers)
			LOG('cly')
		end
		LayoutHelpers.FillParent(UI.Images[c].cursedbtn,UI.Images[c])
		UI.Images[c].cursedbtn.Depth:Set(10)
	end
